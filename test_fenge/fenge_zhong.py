...
def split_audio_by_speech(filename):
    ...
    # 遍历每段说话时间，截取对应的音频并保存
    P_res = open('people_time.txt', 'w', encoding='GBK')
    for i in range(len(start_time_list)):
        start_time = start_time_list[i]
        end_time = end_time_list[i]
        out_file = 'test_fenge/results/{}_{}_{}.wav'.format(i, start_time, end_time)
        extract_audio(filename, out_file, start_time, end_time)
        print('已保存分割后的音频文件：{}'.format(out_file))
        print(start_time, end_time, file=P_res, end='\n')

    P_res.close()
    # 测试代码


def fenge_main(filename='ceshi_30s_lian.wav'):
    for root, dirs, files in os.walk('test_fenge/results'):
        for file in files:
            #print(file)
